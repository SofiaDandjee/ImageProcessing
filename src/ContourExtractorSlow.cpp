...
ContourExtractorSlow::ContourExtractorSlow() {}

ContourExtractorSlow::~ContourExtractorSlow() {}

Channel ContourExtractorSlow::preprocess(const Channel& image) const {
...
Channel ContourExtractorSlow::detectVerticalEdges(const Channel& image) const {
...
Channel ContourExtractorSlow::detectHorizontalEdges(const Channel& image) const {

    Channel filter = {{-1, 0, 1},{-2, 0, 2},{-1, 0, 1}}; //Sobel filter
...
Channel ContourExtractorSlow::convolution(const Channel& image, Channel filter) const {
    size_t l = image.size();
...
Channel ContourExtractorSlow::detectAllEdges(const Channel& image) const {
    size_t l = image.size();
    size_t c = image[0].size();
