...
Run::Run() { r = new ReaderCImg();
w = new WriterCImg();
c = new ContourExtractorSlow();
}
...
void Run::extractContours(const Channel& image) {
    Channel contours = c->detectAllEdges(image);
    w->saveGSImage(contours,"../results/contour.jpeg");
    cout << "Saved as contour.jpeg in the results folder." << endl;
}

void Run::computeFFTandIFFTGS (const Channel& image) {
    ...
    w->saveFFTImage(modulus,"../results/FFT.png" );
    w->saveGSImage(reconstructed,"../results/reconstructed.png");
...
void Run::computeFFTandIFFTColor(std::string filepath) {
    ...
    w->saveFFTImage(modulusRed, "../results/FFT_red.png");
    w->saveFFTImage(modulusGreen, "../results/FFT_green.png");
    w->saveFFTImage(modulusBlue,"../results/FFT_blue.png");
    IFFT(fftRed);
    IFFT(fftGreen);
    IFFT(fftBlue);
    ...
    w->saveRGBImage1(reconstructedR,reconstructedG, reconstructedB, "../results/reconstructed_colored.png");
    cout << "Reconstruction saved as reconstructed_colored.png in the results folder." << endl;
}

void Run::computeHistogram(const Channel& image, string filepath) {
